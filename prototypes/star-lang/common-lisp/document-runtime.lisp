...
(defun current-unix-milliseconds ()
  (* 1000 (unix-now)))
        ...
        (with-open-file (stream #P"/dev/urandom"
          ...
          (unless (= (read-sequence bytes stream) count)
            (fail-runtime 'id-generation-error
                          "Could not read ~D random bytes from /dev/urandom."
                          count)))
      (error ()
        (dotimes (index count)
...
(defun encode-crockford-128 (value)
  (let ((output (make-string 26)))
...
(defun make-ulid (&key (timestamp-ms (current-unix-milliseconds)))
  (unless (and (integerp timestamp-ms)
               (<= 0 timestamp-ms)
               (< timestamp-ms (ash 1 48)))
    (fail-runtime 'id-generation-error
                  "ULID timestamp must fit in 48 unsigned bits, received ~S."
                  timestamp-ms))
  (let* ((random-value (bytes-to-integer (secure-random-bytes 10)))
         (combined (logior (ash timestamp-ms 80) random-value)))
    (encode-crockford-128 combined)))
...
(defun make-uuidv4 ()
  (let ((bytes (secure-random-bytes 16)))
    ...
    (string-downcase
     (format nil "~{~A~}~{-~{~A~}~}"
             (loop for index below 4 collect (byte-hex (aref bytes index)))
             (list
              (loop for index from 4 below 6 collect (byte-hex (aref bytes index)))
              (loop for index from 6 below 8 collect (byte-hex (aref bytes index)))
              (loop for index from 8 below 10 collect (byte-hex (aref bytes index)))
              (loop for index from 10 below 16 collect (byte-hex (aref bytes index))))))))
...
(defun canonical-star-value (value)
  (cond
    ...
    ((characterp value) (with-standard-io-syntax (write-to-string value)))
    ...
    ((hash-table-p value)
     (let ((entries '()))
       (maphash (lambda (key item)
                  (push (cons (identifier-string key) item) entries))
                value)
       (canonical-star-value entries)))
    ...
    (t
     (with-standard-io-syntax (write-to-string value)))))

(defun temporary-id-path ()
  (merge-pathnames
   (make-pathname
    ...
    (unwind-protect
         (progn
           ...
           (let* ((output
                  ...
                  (expected-length (if (eq normalized :md5) 32 64)))
             (unless (and (= (length hex) expected-length)
                          (every (lambda (character)
                                   (or (digit-char-p character)
                                       (find character "abcdef")))
                                 hex))
               (fail-runtime 'id-generation-error
                             "Could not parse ~A output ~S."
                             (digest-program normalized) output))
...
(defun generate-id (kind &key value algorithm prefix timestamp-ms)
  (let ((normalized
          (intern (string-upcase (identifier-string kind)) :keyword)))
    (case normalized
      (:ULID (make-ulid :timestamp-ms
                        (or timestamp-ms (current-unix-milliseconds))))
      (:UUIDV4 (make-uuidv4))
      ...
      (:DIGEST
       (unless algorithm
         (fail-runtime 'invalid-id-policy-error
                       "Digest ID generation requires :algorithm."))
       (make-digest-id algorithm value :prefix prefix))
      (t
       (fail-runtime 'invalid-id-policy-error
                     "Unknown ID generation kind ~S." kind)))))
...
(defun compile-id-policy (form)
  (let ((options (or form '(:kind ulid))))
    (unless (and (listp options) (evenp (length options)))
      (fail-runtime 'invalid-id-policy-error
                    "ID policy must be a property list, received ~S." form))
    (let* ((kind-value (or (getf options :kind) 'ulid))
           (kind (intern (string-upcase (identifier-string kind-value)) :keyword))
           (algorithm-value (getf options :algorithm))
           (algorithm (and algorithm-value
                           (intern (string-upcase
                                    (identifier-string algorithm-value))
                                   :keyword)))
      ...
      (unless (member kind '(:ULID :UUIDV4 :DIGEST :SUPPLIED) :test #'eq)
        (fail-runtime 'invalid-id-policy-error
                      "ID policy kind must be ulid, uuidv4, digest, or supplied."))
      (when (eq kind :DIGEST)
        (unless (member algorithm '(:MD5 :SHA256) :test #'eq)
          (fail-runtime 'invalid-id-policy-error
                        "Digest policy algorithm must be md5 or sha256."))
        (unless fields
          (fail-runtime 'invalid-id-policy-error
                        "Digest policy requires a non-empty :fields list.")))
      (when (and prefix (not (stringp prefix)))
        (fail-runtime 'invalid-id-policy-error
                      "ID policy prefix must be a string."))
      (make-id-policy :kind kind
                      :algorithm algorithm
                      :fields fields
                      :prefix prefix))))

(defstruct document-contract
  qualified-name
  library-name
  library-version
  fields
...
(defstruct (document-instance
            (:constructor %make-document-instance (type values)))
  type
  values)
...
(defun normalize-input-map (value)
  (cond
    ...
    ((hash-table-p value)
     (let ((output '()))
       (maphash (lambda (key item)
                  (push (cons (identifier-string key) item) output))
                value)
       output))
    ((map-like-alist-p value)
     (mapcar (lambda (entry)
    ...
    (t
     (fail-runtime 'invalid-document-error
                   "Document values must be an alist, hash table, or property list."))))
  ...
  (loop for node in (star-lang.loader:loaded-graph-libraries graph)
        do (loop for declaration in
                 ...
                 when (and (string= (or (getf declaration :qualified-name) "")
                   ...
                   do (return-from find-compiled-declaration
          ...
          (cond
            ((null matches)
             (fail-runtime 'unknown-document-error
                           "No document named ~A is loaded." normalized))
            ((cdr matches)
             (fail-runtime 'unknown-document-error
                           "Document name ~A is ambiguous: ~{~A~^, ~}."
                           normalized matches))
            (t (first matches)))))))
  ...
  (let ((cache (make-hash-table :test #'equal))
    ...
    (labels
        ((compile-one (qualified-name)
           (or (gethash qualified-name cache)
               (progn
                 (when (gethash qualified-name active)
                   (fail-runtime 'invalid-document-error
                                 "Document inheritance cycle at ~A."
                                 qualified-name))
                 ...
                 (multiple-value-bind (compiled node)
                   ...
                   (unless compiled
                     (fail-runtime 'unknown-document-error
                                   "No loaded document contract named ~A."
                                   qualified-name))
                   (let* ((raw (raw-document-declaration node (getf compiled :name)))
                          ...
                          (policy-form
                            (and (plist-key-present-p options :id-policy)
                          ...
                          (contract
                            (make-document-contract
                             ...
                             :library-name
                             (star-lang.loader:library-node-name node)
                             :library-version
                             (star-lang.loader:library-node-version node)
                             :fields
                             (merge-fields
                              (if parent
                     ...
                     (setf (gethash qualified-name cache) contract)
                     contract))))))
  ...
  (cond
    ...
    ((stringp type)
     (cond
       ...
       ((string= type "map") (map-value-p value))
       ((string= type "reference")
        (or (stringp value) (map-value-p value)
            (typep value 'document-instance)))
       ((member type '("string" "symbol" "iso-date" "iso-datetime")
                :test #'string=)
        (or (stringp value) (and (string= type "symbol") (symbolp value))))
       (t
...
(defun validate-document (graph document &optional contract)
  (let* ((resolved-contract
           (or contract
               (compile-document-contract graph (document-instance-type document))))
         (values (document-instance-values document))
         (known-fields
           (mapcar (lambda (field) (getf field :name))
                   (document-contract-fields resolved-contract))))
    (dolist (entry values)
      (unless (member (car entry) known-fields :test #'string=)
        (fail-runtime 'invalid-document-error
                      "Unknown field ~A for ~A."
                      (car entry)
                      (document-contract-qualified-name resolved-contract))))
    (dolist (field (document-contract-fields resolved-contract))
      (let ((name (getf field :name)))
        (when (and (getf field :required)
                   (not (map-has-key-p values name)))
          (fail-runtime 'invalid-document-error
                        "Required field ~A is missing from ~A."
                        name
                        (document-contract-qualified-name resolved-contract)))
        (when (map-has-key-p values name)
          (let ((value (map-value values name)))
            (unless (value-matches-type-p graph value (getf field :type))
              (fail-runtime 'invalid-document-error
                            "Field ~A in ~A does not match type ~S: ~S."
                            name
                            (document-contract-qualified-name resolved-contract)
                            (getf field :type)
                            value))))))
...
(defun policy-input (policy values)
  (mapcar (lambda (field)
...
(defun assign-policy-id (policy values)
  (case (id-policy-kind policy)
    ...
    (:UUIDV4 (make-uuidv4))
    (:DIGEST
     (make-digest-id (id-policy-algorithm policy)
                     (policy-input policy values)
    ...
    (t
     (fail-runtime 'invalid-id-policy-error "Unhandled ID policy."))))
...
(defun set-document-meta (document contract &key dataset)
  (let* ((values (document-instance-values document))
    ...
    (unless (and (stringp resolved-dataset) (> (length resolved-dataset) 0))
      (fail-runtime 'invalid-document-error
                    "Creating ~A requires a non-empty dataset."
...
(defun create-document (graph document-type values &key dataset (validate t))
  (let* ((contract (compile-document-contract graph document-type))
         (normalized (apply-field-defaults contract (normalize-input-map values)))
         (document (%make-document-instance
                    (document-contract-qualified-name contract)
                    normalized)))
...
(defun encoded-key (key key-style couchdb)
  (cond
    ...
    ((eq key-style :snake) (snake-key key))
    (t key)))
        ...
        unless (and couchdb
          ...
          (cons (encoded-key key key-style couchdb)
                (cond
                  ...
                  ((listp value)
                   (mapcar (lambda (item)
                             (if (typep item 'document-instance)
                                 (encode-document item
                                                  :key-style key-style
                                                  :couchdb couchdb)
                                 item))
                           value))
    ...
    (t
     (let ((match
             (find external-key
                   ...
                   :key (lambda (field)
                          (encoded-key (getf field :name) key-style couchdb))
                   :test #'string=)))
       (and match (getf match :name))))))
         ...
         (values
           (loop for (external-key . value) in input
                 ...
                 do (unless internal-key
                      (fail-runtime 'invalid-document-error
                                    "Encoded key ~A is not valid for ~A."
                                    external-key
                                    (document-contract-qualified-name contract)))
...
(defun relate-documents (graph source target
  ...
  (unless (and (typep source 'document-instance)
    ...
    (fail-runtime 'invalid-document-error
                  "relate-documents requires two document instances."))
  (create-document
   graph relation-type
   (list
