        ...
        (with-open-file (stream #P"/dev/urandom"
          ...
          (unless (= (read-sequence bytes stream) count)
            (fail-runtime 'id-generation-error
                          "Could not read ~D bytes from /dev/urandom."
                          count)))
      (error ()
        (dotimes (index count)
...
(defun generate-id (kind &key value algorithm prefix timestamp-ms)
  (case (intern (string-upcase (identifier-string kind)) :keyword)
    (:ULID
     (make-ulid :timestamp-ms
                (or timestamp-ms (current-unix-milliseconds))))
    (:UUIDV4
     (make-uuidv4))
    (:MD5
     (make-digest-id :md5 value :prefix prefix))
    (:SHA256
     (make-digest-id :sha256 value :prefix prefix))
    (:DIGEST
     (unless algorithm
...
(defun compile-id-policy (form)
  (let ((options (or form '(:kind ulid))))
    (unless (and (listp options) (evenp (length options)))
      (fail-runtime 'invalid-id-policy-error
                    "ID policy must be a property list, received ~S."
                    form))
    (let* ((kind
             (intern
              (string-upcase
              ...
              :keyword))
           (algorithm
             (and (getf options :algorithm)
                  (normalized-algorithm (getf options :algorithm))))
           (fields
             (mapcar #'identifier-string (or (getf options :fields) '())))
      ...
      (when (eq kind :DIGEST)
        (unless algorithm
          (fail-runtime 'invalid-id-policy-error
                        "Digest ID policy requires md5 or sha256."))
        (unless fields
          (fail-runtime 'invalid-id-policy-error
                        "Digest ID policy requires :fields.")))
                 ...
                 (multiple-value-bind (compiled node)
                   ...
                   (unless compiled
                     (fail-runtime 'unknown-document-error
                                   ...
                                   qualified-name))
                   (let* ((raw
                            (raw-document-declaration node
                                                      (getf compiled :name)))
                          (options (third raw))
                          (parent-name (getf compiled :extends))
                          (parent
                            (and parent-name (compile-one parent-name)))
                          (policy-form
                          ...
                          (policy
                            (cond
                              (policy-form
                               (compile-id-policy policy-form))
                              (parent
                               (document-contract-id-policy parent))
                              (t
                               (compile-id-policy '(:kind ulid)))))
                          (contract
  ...
  (cond
    ...
    (t
     (let ((declaration (declaration-for-type graph type)))
       (unless declaration
         (fail-runtime 'invalid-document-error
                       "Unknown field type ~A."
                       type))
       (case (getf declaration :kind)
         (:scalar
          (and
           (value-matches-type-p graph value (getf declaration :base))
...
(defun validate-document (graph document &optional contract)
  (let* ((resolved-contract
           (or contract
               (compile-document-contract
                graph
                (document-instance-type document))))
         (values (document-instance-values document))
         (fields (document-contract-fields resolved-contract))
         (known-fields
           (mapcar (lambda (field) (getf field :name)) fields)))
    (dolist (entry values)
...
(defun encoded-key (key key-style couchdb)
  (cond
    ((and couchdb (string= key "id"))
     "_id")
    ((and couchdb (string= key "rev"))
     "_rev")
    ((eq key-style :camel)
     (camel-key key))
    ((eq key-style :snake)
...
(defun encode-document (document &key (key-style :camel) (couchdb t))
  (loop for (key . value) in (document-instance-values document)
        unless (and couchdb
          ...
          collect
          (cons
           (encoded-key key key-style couchdb)
           (cond
             ((typep value 'document-instance)
...
(defun decoded-field-name (contract external-key key-style couchdb)
  (cond
    ((and couchdb (string= external-key "_id"))
     "id")
    ((and couchdb (string= external-key "_rev"))
     "rev")
    (t
     (let ((field
             (find external-key
