  ...
  (let* ((local-projects
    ...
    (write-lines
     ...
     '("(defpackage #:cl-couchdb-client"
       ...
       ""
       "(defun server-request-parts (server)"
       "  (let* ((scheme-end (search \"://\" server))"
       "         (userinfo-start (and scheme-end (+ scheme-end 3)))"
       "         (at-position"
       "           (and userinfo-start"
       "                (position (code-char 64) server :start userinfo-start))))"
       "    (if (null at-position)"
       "        (values server nil)"
       "        (let* ((userinfo (subseq server userinfo-start at-position))"
       "               (colon (position (code-char 58) userinfo)))"
       "          (unless colon"
       "            (error \"CouchDB userinfo requires username:password.\"))"
       "          (values"
       "           (concatenate 'string"
       "                        (subseq server 0 userinfo-start)"
       "                        (subseq server (1+ at-position)))"
       ...
       "  (multiple-value-bind (base-server basic-authorization)"
       "      (server-request-parts server)"
       "    (let* ((uri (format nil \"~A~{/~A~}~A\""
       "                        base-server path (or (query-string keys) \"\")))"
       "           (arguments"
       "             (list uri"
       "                   :method method"
       "                   :force-binary nil"
       ...
       "        (setf arguments"
       "              (append arguments"
       "                      (list :basic-authorization basic-authorization))))"
       "      (when content"
       "        (setf arguments"
       "              (append arguments"
       "                      (list :content"
