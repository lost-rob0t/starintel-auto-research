       ...
       (progn
         ...
         (let* ((first-command
           ...
           (unless (string= first-stdout replay-stdout)
             (error "Remote run-id replay reran the tool: first ~S replay ~S."
           ...
           (let* ((conflict-command
                  ...
                  (error-envelope
                    (smoke-envelope-of-kind
                     (smoke-submit-deferred
                      runtime conflict-command "remote run-id conflict")
                     :error)))
             (unless (string= (getf error-envelope :code)
                              "star.bbp.run-id-conflict")
               (error "Unexpected remote conflict envelope ~S."
                      error-envelope))))
         (smoke-write-marker "bbp-sento-smoke.success" "ok")
         (format t "Two-process Sento BBP replay smoke test passed.~%"))
